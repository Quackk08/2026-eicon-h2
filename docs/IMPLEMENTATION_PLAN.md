...
- 모바일 브라우저를 포함한 반응형 PWA와 터치 입력 지원
...
- 모바일은 별도 축소 버전이 아니라 모든 MVP 핵심 기능을 제공하는 기본 지원 환경이다.
- 320px 이상 화면에서 가로 스크롤 없이 사용할 수 있어야 한다.
- 모바일, 태블릿, 데스크톱은 같은 정보 구조를 유지하고 화면 크기에 따라 배치와 노출 순서만 조정한다.
- 주요 흐름은 세로 한 열을 우선하며 보조 정보는 탭, 접기, 단계별 노출 방식으로 정리한다.
- 모든 주요 터치 영역은 최소 44x44px을 확보한다.
- hover에 의존하는 기능을 만들지 않고 터치, 키보드, 포인터 입력을 모두 지원한다.
- 모바일 입력 필드는 iOS 자동 확대를 피할 수 있는 읽기 가능한 크기를 유지한다.
- 하단 고정 UI는 `safe-area-inset-*`을 고려해 브라우저와 기기 UI에 가리지 않도록 한다.
- iOS Safari와 Android Chrome을 우선 지원하며 모바일 가로 및 세로 방향을 모두 확인한다.
- PWA 설치 여부와 관계없이 브라우저에서 전체 기능을 사용할 수 있어야 한다.
- 느린 네트워크와 오프라인 상태에서 Check-In, Mission 확인, Reflection 작성이 유지되어야 한다.
- 디자인 레퍼런스가 전달되기 전에는 모바일 내비게이션의 최종 형태와 시각적 breakpoint를 확정하지 않는다.
...
- 모바일 viewport, safe area, 터치 입력, 설치 모드 기본 검증
...
- 320px, 375px, 390px, 768px, 1024px 기준 화면
- iOS Safari와 Android Chrome의 세로 및 가로 모드
- 터치, 키보드, 포인터 입력
- 모바일, 태블릿, 데스크톱 브라우저
